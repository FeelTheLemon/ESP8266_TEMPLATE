...
function createTree(k, l) {
	...
	function createFileTreeLeaf(a, b, c) {
		var d = document.createElement("li");
		d.id = (((a == "/") ? "": a) + "/" + b);
	...
	function createDirTreeLeaf(a, b, c) {
		var d = document.createElement("li");
		d.id = (((a == "/") ? "": a) + "/" + b);
		var f = document.createElement("span");
		f.innerText = b;
	...
	function addList(a, b, c) {
		...
		for (var i = 0; i < e; i++) {
			if (c[i].type === "file")
				d.appendChild(createFileTreeLeaf(b, c[i].name, c[i].size))
			
			if (c[i].type === "dir")
				d.appendChild(createDirTreeLeaf(b, c[i].name, c[i].size))
		}
	}
	...
	function getCb(a, b) {
		return function () {
			if (xmlHttp.readyState == 4) {
				if (xmlHttp.status == 200)
				addList(a, b, JSON.parse(xmlHttp.responseText))
			}
		}
	}
